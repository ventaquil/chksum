use super::block::{Block, BLOCK_LENGTH_BYTES};

#[rustfmt::skip]
const PADDING: [u8; BLOCK_LENGTH_BYTES] = [
    ,
];

#[derive(Debug, Eq, PartialEq)]
pub(super) enum Padding {
    Single(Block),
    Double(Block, Block),
}

impl From<Block> for Padding {
    fn from(block: Block) -> Self {
        Self::Single(block)
    }
}

impl From<(Block, Block)> for Padding {
    fn from(blocks: (Block, Block)) -> Self {
        Self::Double(blocks.0, blocks.1)
    }
}

pub(super) fn pad(block: Block, block_length: usize, counter: usize) -> Padding {
    assert!(block_length <= BLOCK_LENGTH_BYTES);
    assert!(block_length <= counter);

    #[rustfmt::skip]
    let length: [u8; BLOCK_LENGTH_BYTES] = {
        let length = counter as u128;
        let length = length * 8; // convert byte-length into bits-length
        let length = length.to_be_bytes();
        [
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00, 
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00, 
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00, 
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00, 
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,
                   0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,        0x00,
            length[0x0], length[0x1], length[0x2], length[0x3], length[0x4], length[0x5], length[0x6], length[0x7],
            length[0x8], length[0x9], length[0xA], length[0xB], length[0xC], length[0xD], length[0xE], length[0xF],
        ]
    };
    let padding: Block = PADDING.into();
    if (block_length + 1 + 8) <= BLOCK_LENGTH_BYTES {
        let padding = padding >> block_length;
        (block | padding | length).into()
    } else if block_length < BLOCK_LENGTH_BYTES {
        // block_length + 1 <= BLOCK_LENGTH_BYTES
        let padding = padding >> block_length;
        let length: Block = length.into();
        (block | padding, length).into()
    } else {
        (block, padding | length).into()
    }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_empty() {
        assert_eq!(
            pad(
                [
                    ,
                ]
                .into(),
                0x00,
                0x00
            ),
            Padding::Single(
                [
                    ,
                ]
                .into()
            )
        );
    }

    #[test]
    fn test_single_block() {
        assert_eq!(
            pad(
                [
                    ,
                ]
                .into(),
                0x08,
                0x08
            ),
            Padding::Single(
                [
                    ,
                ]
                .into()
            )
        );

        assert_eq!(
            pad(
                [
                    ,
                ]
                .into(),
                0x10,
                0x10
            ),
            Padding::Single(
                [
                    ,
                ]
                .into()
            )
        );

        assert_eq!(
            pad(
                [
                    ,
                ]
                .into(),
                0x20,
                0x20
            ),
            Padding::Single(
                [
                    ,
                ]
                .into()
            )
        );

        assert_eq!(
            pad(
                [
                    ,
                ]
                .into(),
                0x30,
                0x30
            ),
            Padding::Single(
                [
                    ,
                ]
                .into()
            )
        );

        assert_eq!(
            pad(
                [
                    ,
                ]
                .into(),
                0x37,
                0x37
            ),
            Padding::Single(
                [
                    ,
                ]
                .into()
            )
        );
    }

    #[test]
    fn test_double_block() {
        assert_eq!(
            pad(
                [
                    ,
                ]
                .into(),
                0x78,
                0x78
            ),
            Padding::Double(
                [
                    ,
                ]
                .into(),
                [
                    ,
                ]
                .into()
            )
        );

        assert_eq!(
            pad(
                [
                    ,
                ]
                .into(),
                0x7F,
                0x7F
            ),
            Padding::Double(
                [
                    ,
                ]
                .into(),
                [
                    ,
                ]
                .into()
            )
        );

        assert_eq!(
            pad(
                [
                    ,
                ]
                .into(),
                0x80,
                0x80
            ),
            Padding::Double(
                [
                    ,
                ]
                .into(),
                [
                    ,
                ]
                .into()
            )
        );
    }
}
