...
mod sha2 {
    ...
    mod sha224 {
        ...
        fn empty_directory() -> Result {
            ...
            let digest = read_dir(&directory)?.chksum(HashAlgorithm::SHA2_224)?;
            let digest = format!("{:X}", digest);
    ...
    mod sha256 {
        ...
        fn empty_directory() -> Result {
            ...
            let digest = read_dir(&directory)?.chksum(HashAlgorithm::SHA2_256)?;
            let digest = format!("{:X}", digest);
            assert_eq!(
                digest,
    ...
    mod sha512 {
        ...
        fn empty_directory() -> Result {
            ...
            let digest = read_dir(&directory)?.chksum(HashAlgorithm::SHA2_512)?;
            let digest = format!("{:X}", digest);
            assert_eq!(
                digest,
